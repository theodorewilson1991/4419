...
RoutingTable::AddRoute (RoutingTableEntry & rt)
{
  ...
  //Q3 Here
  DeleteRandomNode(0.4 /*this is the prob*/);
...
void
RoutingTable::DeleteRandomNode(double prob) {
    //add call to the start of Update and AddRoute

    //seed random number generator with current time
    std::srand(std::time(0));
    bool success;
    double randd = ((double) std::rand() / (RAND_MAX));

    if ((randd <= prob) && !m_ipv4AddressEntry.empty()) {

        //random int within table size
        int choice = std::rand() % m_ipv4AddressEntry.size();

        std::map<Ipv4Address, RoutingTableEntry>::iterator it = m_ipv4AddressEntry.begin();
        for (int i = 0; i < choice-1; i++) {
            it++;
        }
        //ip address to delete
        Ipv4Address todel = it->first;
        success = RoutingTable::DeleteRoute(todel);

        if (success) {
            printf("Deleted address \n");
            todel.Print(std::cout);
            std::cout << "\n";
        } else {
            printf("could not delete chosen item\n");
        }
